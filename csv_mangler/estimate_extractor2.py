...
This is a bit of a refinement on estimate_extractor.py - here we at least take factor names
from text files specified as arguments. We here expect to extract pairs of factors that are
represented in contiguous entries in the estimate data ACS file. In practice this rarely
happens, leading to the various estimate_extractor_edu, etc files.
"""
import os
import itertools
import lookup
    ...
    for line in f:
        ...
        if sequence == args.sequence and logrec in RELEVANT_LOCRECS:
            ...
            for n1,n2 in factors:
                val = int(toks[idx])
                idx += 1
                print '%s,%s,%s,%d,%s'%(args.rectype,location,n2,val,n1)
